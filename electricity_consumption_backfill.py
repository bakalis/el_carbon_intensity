...
df_raw = read_csv_folder(
    Path("data/electricity-consumption"), parse_datetime_cols=["Datetime (UTC)"]
)
...
con_expectation_suite = ge.core.ExpectationSuite(
    expectation_suite_name="con_expectation_suite"
)

# Add expectations for load column: values between 0 and 50000
con_expectation_suite.add_expectation(
    ge.core.ExpectationConfiguration(
        expectation_type="expect_column_values_to_be_between",
        kwargs={"column": "load", "min_value": 0, "max_value": 50000, "mostly": 1.0},
    )
)
...
electricity_consumption_fg.insert(con_features, wait=True)

electricity_consumption_fg.update_feature_description(
    "datetime", "Date and time of electricity load data point (utc)"
)
electricity_consumption_fg.update_feature_description(
