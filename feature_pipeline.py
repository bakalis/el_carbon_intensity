...
from helpers.utils import em_zone_to_entsoe_zone, datetime_to_unix
...
for zone in settings.ZONES:
    ...
    start = pd.to_datetime(now - timedelta(hours=3), utc=True)
    end = pd.to_datetime(now, utc=True)
    zone_data = entsoe_client.query_generation(
        country_code=entsoe_zone, start=start, end=end
    )
...
df = df.sort_values(by="datetime")
electricity_generation_fg.insert(df)
...
for zone in settings.ZONES:
    ...
    df = pd.concat([df, zone_data], ignore_index=True)
df = df.sort_values(by="datetime")
electricity_consumption_fg.insert(df)
...
for zone in settings.ZONES:
    ...
    zone_import = entsoe_client.query_physical_crossborder_allborders(
        country_code=entsoe_zone, start=start, end=end, export=False, per_hour=True
    )["sum"]
    zone_export = entsoe_client.query_physical_crossborder_allborders(
        country_code=entsoe_zone, start=start, end=end, export=True, per_hour=True
    )["sum"]
