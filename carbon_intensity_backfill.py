...
ci_features = (
    ...
    .filter(
        ~pl.any_horizontal(pl.col(pl.FLOAT_DTYPES).is_nan())
    )
    .sort("datetime")
)
...
carbon_intensity_fg.update_feature_description("datetime", "Date and time of carbon intensity data point")
carbon_intensity_fg.update_feature_description("zone_id", "Id of the region the data point is valid for")
carbon_intensity_fg.update_feature_description("ci_direct", "Emissions released directly from the operation of the power plant")
carbon_intensity_fg.update_feature_description("ci_lifecycle", "Emissions from building the power plant, operating it, extracting fuel, and disposal at the end of its life")
carbon_intensity_fg.update_feature_description("data_estimated", "Whether or not the value was calculated based on actual Entso-E data")